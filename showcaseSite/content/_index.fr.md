---
title: "Hello Friend AI"
description: "Toutes les options du thème activées d'un coup, en quatre langues, avec un article sur ce que coûte chacune."
---

Ce paragraphe vient de `content/_index.md`. Tout ce qui manque autrement à une
page d'accueil se met ici : ce dont parle le site, qui l'écrit et par où
commencer.

Ceci est la vitrine. Toutes les options du thème y sont activées d'un coup, en
quatre langues, et chaque article dit ce que coûte l'une d'elles. La
[démo par défaut](https://mehdilaruelle.github.io/hugo-theme-hello-friend-ai/)
les laisse toutes éteintes, pour qu'un thème ressemble à lui-même dès
l'installation plutôt qu'à une liste de fonctionnalités.

C'est du Markdown ordinaire, rendu entre le sous-titre et les icônes sociales.
Un site qui n'en veut pas n'a tout simplement pas de `_index.md`, ce que fait
la démo.

## Par où commencer

- [Chaque interrupteur et ce qu'il coûte]({{< ref "posts/switches.md" >}})
- [À propos]({{< ref "about.md" >}})
